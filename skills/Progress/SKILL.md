...
Read `rules/Levels.md`. Count checked (`- [x]`) and unchecked (`- [ ]`) boxes per level. A level is **complete** when all 3 of its checkboxes are checked.
...
- "Install a second module — try forge-text for text processing, forge-council for code review, or forge-avatar for deep identity."
