...
Read `rules/Identity.md` for the user's name, preferences, and experience level. Adjust your communication style accordingly.

Read `rules/Goals.md` for what the user is currently working toward. Reference their goals when suggesting projects or priorities.

Read any other `.md` files in `rules/` — the user may add custom rules or preferences there (e.g., `rules/MyRules.md`). Follow them.
...
This project has a 7-level progression. Read `rules/Levels.md` for the roadmap and current progress.
...
```
rules/       — who the user is (templates checked in, personal .md files generated by make install and gitignored)
...
Read the actual rules files (`rules/Identity.md`, `Goals.md`, `Levels.md`), not the `.template` files. If a rules file is missing, tell the user to run `make install`.
