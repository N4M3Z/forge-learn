...
```
steering/    — who the user is (templates checked in, personal .md files generated by make install and gitignored)
...
Read the actual steering files (`steering/Identity.md`, `Goals.md`, `Levels.md`), not the `.template` files. If a steering file is missing, tell the user to run `make install`.
